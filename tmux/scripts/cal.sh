...
get_attendees() {
	...
	icalBuddy \
		...
		--excludeEndDates \
		--bullet "" \
		--excludeCals "training,mwamodojnr@gmail.com" \
		eventsToday)
}
...
get_next_next_meeting() {
	end_timestamp=$(python3 - "$end_time" <<'PY'
from datetime import datetime
import sys

end_time = sys.argv[1]
now = datetime.now().astimezone()
dt = datetime.strptime(f"{now:%Y-%m-%d} {end_time}:01", "%Y-%m-%d %H:%M:%S").replace(tzinfo=now.tzinfo)
...
t = sys.argv[1]
now = datetime.now()
dt = datetime.strptime(f"{now:%Y-%m-%d} {t}", "%Y-%m-%d %H:%M:%S")
...
calculate_times(){
	if [[ -z "$time" ]]; then
		...
		epoc_now=$(date +%s)
		epoc_diff=0
		minutes_till_meeting=99999
		return
	fi

	epoc_meeting=$(to_epoch_today "$time:00")
	epoc_now=$(date +%s)
